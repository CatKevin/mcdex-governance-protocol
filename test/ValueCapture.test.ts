...
describe('ValueCapture', () => {
    ...
    it("valueCapture - admin", async () => {
        ...
        await expect(valueCapture.connect(user1).forwardMultiAssets([mcb.address], [toWei("100")])).to.be.revertedWith("caller is not authorized");
        await valueCapture.forwardMultiAssets([mcb.address], [toWei("100")])
    ...
    it("exception - forward", async () => {
        ...
        await expect(valueCapture.forwardMultiAssets([mcb.address], [toWei("100")])).to.be.revertedWith("amount in execceds convertable amount")
        // no converter
        await mcb.mint(valueCapture.address, toWei("100"))
        await expect(valueCapture.forwardMultiAssets([mcb.address], [toWei("0")])).to.be.revertedWith("amount in is zero")
        await expect(valueCapture.forwardMultiAssets([mcb.address], [toWei("100")])).to.be.revertedWith("token exchange is not available")
