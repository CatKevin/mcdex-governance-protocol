...
describe("Minter", () => {
  ...
  beforeEach(async () => {
    ...
    tm = await createContract("TimeMachine");
    await tm.turnOn();

    mcb = await createContract("CustomERC20", ["MCB", "MCB", 18]);
    minter = await createContract("TestMCBMinter", [tm.address]);
    await minter.initialize(auth.address, mcb.address, user1.address, 100, toWei("2000000"), toWei("0.2"));
    valueCapture = await createContract("MockValueCapture", [minter.address]);
  ...
  const caseTester = async (contracts, cases, isNonStatic = false) => {
    const runCase = async (element, isNonStatic) => {
      ...
      if (isDefined(element.block)) {
        ...
        await contracts.tm.setBlockNumber(element.block);
      }
  ...
  it("reinitialize", async () => {
    await expect(
      minter.initialize(auth.address, mcb.address, user1.address, 100, toWei("2000000"), toWei("0.2"))
    ).to.be.revertedWith("contract is already initialized");
  });

  it("base release", async () => {
    await tm.setBlockNumber(1); // < 100
    await caseTester({ tm, minter, valueCapture }, [
      {
        base: 0,
        extra: 0,
      },
  ...
  it("base mint", async () => {
    await tm.setBlockNumber(1110); // < 100
    const { baseMintableAmount } = await minter.callStatic.getMintableAmounts();
    expect(baseMintableAmount).to.equal(toWei("202"));
    {
      await expect(minter.connect(user2).mintFromBase(user2.address, toWei("200"))).to.be.revertedWith(
    ...
    }
    {
      ...
      await expect(minter.mintFromBase(user3.address, toWei("1"))).to.be.revertedWith(
        "amount exceeds max mintable amount"
      );
    }
    {
      // surpass max base supply
      await tm.setBlockNumber(50000100); // < 100
      const { baseMintableAmount } = await minter.callStatic.getMintableAmounts();
  ...
  it("round mint - 2", async () => {
    await tm.setBlockNumber(1); // < 100
    await minter.newRound(user2.address, toWei("700000"), toWei("0.5"), 110);
    await caseTester({ tm, minter, valueCapture }, [
      {
        base: 0,
        extra: 0,
  ...
  it("round mint - more rounds | static", async () => {
    ...
    await minter.newRound(user2.address, toWei("700000"), toWei("0.5"), 110);
    // round - 1
    await minter.newRound(user2.address, toWei("200000"), toWei("0.2"), 150);
    await caseTester({ tm, minter, valueCapture }, [
      {
        base: 0,
        extra: 0,
